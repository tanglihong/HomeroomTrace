...
function RecordListInner() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const hasExtraFilter = Boolean(queryFilter.type || queryFilter.startDate || queryFilter.endDate || queryFilter.keyword || queryFilter.studentId);
        if (!hasExtraFilter && cachedRecords) {
  ...
  }, [container, cachedRecords, queryFilter, toast]);
